...
exports.getCityData = function(city, state, business_type, query, pagetoken, cb) {
	...
	function getGoogle(error, response, body, lat, lon, pagetoken) {
		request({
			uri: "https://maps.googleapis.com/maps/api/place/textsearch/json?query=" + query + "&location=" + lat + "," + lon + "&type=" + business_type + "&pagetoken=" + pagetoken + "&rankby=prominence&radius=20&sensor=false&key=AIzaSyATJzyl_9H5NGT1hOFkP5k7ZZSB9JSCiDw",
			method: "GET", 
			JSON: true
